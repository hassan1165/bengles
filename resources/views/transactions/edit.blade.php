...
@section('content')
    @include('transactions.partials.header', ['title' => __('Edit Transaction')])
                        ...
                        <form method="post" action="{{ route('transactions.update', $transaction) }}" autocomplete="off">
                            ...
                            <div class="pl-lg-4">
                                ...
                                <div class="form-group{{ $errors->has('account_id') ? ' has-danger' : '' }}">
                                    ...
                                    <select name="account_id" class="form-control">
                                        ...
                                        @foreach($accounts as $account)
                                            <option value="{{ $account->id }}" {!! ($transaction->account_id==$account->id?"selected":"") !!}>{{ $account->title }}</option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="form-group{{ $errors->has('amount') ? ' has-danger' : '' }}">
                                    ...
                                    <input type="text" name="amount" id="input-amount" class="form-control form-control-alternative{{ $errors->has('amount') ? ' is-invalid' : '' }}" placeholder="{{ __('Amount') }}" value="{{ old('amount', $transaction->amount) }}" required autofocus>
                                ...
                                <div class="form-group{{ $errors->has('details') ? ' has-danger' : '' }}">
                                    ...
                                    <input type="text" name="details" id="input-details" class="form-control form-control-alternative{{ $errors->has('details') ? ' is-invalid' : '' }}" placeholder="{{ __('Detail') }}" value="{{ old('details', $transaction->details) }}" required autofocus>
                                ...
                                <div class="form-group{{ $errors->has('added_by') ? ' has-danger' : '' }}">
                                    ...
                                    <input type="text" name="added_by" id="input-added_by" class="form-control form-control-alternative{{ $errors->has('added_by') ? ' is-invalid' : '' }}" placeholder="{{ __('Added By') }}" value="{{ old('added_by', $transaction->added_by) }}" required autofocus>
